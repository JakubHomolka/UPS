...
void Response::sendErrorToPlayer(Player *player, string errorMessage){
    ...
    errorMessage = player->name + "|" + "ERR|" + errorMessage +"| invalid message counter is |"+ to_string(player->invalid_message_counter);
...
void Response::sendErrorToPlayer1(Player *player, string errorMessage) {
    errorMessage = "LOGIN|ERR|" + errorMessage +"| invalid message counter is |"+ to_string(player->invalid_message_counter);
    player->msg_out = errorMessage + "\n";
    sendToClient(player->socket, errorMessage);
